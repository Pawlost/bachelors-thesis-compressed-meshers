...
#include "Generator/VoxelGeneratorBase.h"
#include "Mesh/RealtimeMeshBuilder.h"
#include "MesherUtils/MesherVariables.h"
...
/**
 * 
 * @param Chunk Voxel model where the voxel data will be saved
 * @param VoxelGrid Voxel data to be inserted. All data for mesher must be compressed from a voxel grid.
 */
	...
	for (uint8 f = 0; f < CHUNK_FACE_COUNT; f++)
	{
		...
		for (const auto Voxel : MeshVars.VoxelIdToLocalVoxelMap)
		{
			...
			if (FaceArray == nullptr || !FaceArray.IsValid())
			{
				...
				FaceArray = MakeShared<TArray<FVoxelMergeFace>>();
                                              ...
                                              const TSharedPtr<FChunkParams>& ChunkParams) const
{
	...
	TWeakObjectPtr<ARMCVoxelModelActor> ActorPtr = Chunk->ChunkMeshActor;
	...
	if (ActorPtr == nullptr)
	{
		...
		ActorPtr = World->SpawnActor<ARMCVoxelModelActor>(ARMCVoxelModelActor::StaticClass(), SpawnLocation,
